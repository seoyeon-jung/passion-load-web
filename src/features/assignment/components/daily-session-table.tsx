...
export function DailySessionTable({ sessionId, studentId }: Props) {
  ...
  if (dates.length === 0)
    return <EmptyState message="등록된 과제가 없습니다." />;

  console.log('assignments raw', assignments);
          ...
          {dates.map((date) => {
            ...
            return items.map((assignment, idx) => (
              <tr key={assignment.id} className="border-b hover:bg-gray-50">
                ...
                <td className="px-4 py-3">{/* 플래너 업로드 - 다음 이슈 */}</td>
                <td className="px-4 py-3">{/* 학생 특 - 다음 이슈 */}</td>
                <td className="px-4 py-3">{/* 강사 피드백 - 다음 이슈 */}</td>
                <td className="px-4 py-3">
                  <div className="flex flex-col gap-1">
                    <span className="text-xs text-gray-400">미이수 사유</span>
